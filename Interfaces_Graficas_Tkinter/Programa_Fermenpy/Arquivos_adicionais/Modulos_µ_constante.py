...
def entr_rand_mi_const():
    def entr_rand_mi_const_gerand():
        mi = 0.35 #unidade 1/hora - taxa específica de crescimento
        Yxs = 0.3  #unidade g células/g substrato - coeficiente estequiométrico
        Kd = 0.025 #unidade de 1/h - constante de morte celular
        Cx0 = 0.5 # unidade g/L - concentração inicial de microrganismo
        Cs0 = 40 # unidade g/L - concentração inicial de substrato
        Cp0 = 0  # unidade g/L - concentração inicial de produto
        tf = 15 # unidade horas - tempo final da integração
        alfa=  0.1 # unidade g células/g produto - coeficiente estequiométrico
        beta = 0.04
...
def modelag_bat_mi_const_dados_conc_sim():
   importado = pd.read_excel("Dados_conc_sim_bat_mi_const_modelag.xlsx","C_exp") 
