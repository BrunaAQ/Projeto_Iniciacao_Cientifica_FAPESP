# Importação das bibliotecas e pacotes: ##EVENTUAIS PROBLEMAS LEMBRAR DO C_EXP=ABS()
...
Label(frame46, text = "Descrição completa em:", font = "Times 8 bold", fg = "black", bg = "gray90").place(x = 5, y = 120)
Button(frame46, text="https://brunaaq.github.io/Documentacao_fermenpy/#ausencia%20inibicao", font = "calibri 8", fg = "blue", bg = "gray80", relief = "raised", command=lambda: webbrowser.open('https://brunaaq.github.io/Documentacao_fermenpy/#ausencia%20inibicao')).place(x = 132, y = 119)
image_docum(imagem = "Equacao_mi_const.png", num_frame = frame46, borda = 5, x = 120, y = 200)
...
# Combobox:
v_2 = ("AUSÊNCIA DE INIBIÇÃO", "INIBIÇÃO PELO SUBSTRATO", "INIBIÇÃO PELO PRODUTO", "INIBIÇÃO PELA BIOMASSA")
...
sai_resul_simul .place(x = 1072, y = 63)

# Notebook para plotagem dos gráficos:
## Simulação:
notebook_graf_model()
...
def equac_sai_mi_const():
    ...
    Label(frame1, text = u"\u03bc constante", font = 'Batang 10 bold', fg = "black", bg = "white").place(x = 930, y = 140)
    tex_mi = Label(frame1, text = u"\u03bc = taxa específica de crescimento (t\u207b\u00b9)", font = 'arial 9 italic', fg = "black", bg = "grey65").place(x = 930, y = 264)
    tex_mi_explic = Label(frame1, text = "A velocidade de crescimento microbiano mantém-se\nconstante durante todo o tempo t de cultivo", font = 'arial 9 italic', fg = "black", bg = "grey65", relief = "sunken").place(x = 936, y = 315)
...
def simulacao(cont):
    ...
    if cont == 9:
        def edos_int_bat_mi_const(C,t):
            ...
            return (dCxdt,dCsdt,dCpdt)
        # Integrando numericamente:
        C_sim = odeint(edos_int_bat_mi_const,inic_cond_simul,t_simul)
        print(C_sim)
        # Condicional para evitar valores negativos para Cs:
        #ContadorCs=0
        Cx = C_sim[:,0]
        Cs = C_sim[:,1]
        Cp = C_sim[:,2]
        for ContadorCs in range(len(t_simul)):
            if (Cs[ContadorCs]<0):
                Cs[ContadorCs]=0
                Cx[ContadorCs]=Cx[ContadorCs-1]
                Cp[ContadorCs]=Cp[ContadorCs-1]
    ...
    def imprimir_perfil_concentracao_model_otim_exp (t_m, Cx_m, Cs_m, Cp_m):
        ...
        _ = lns2 = plot.plot(t_m,Cp_m, linestyle="--", color=p,linewidth=3,label='Cp modelo')
        ax2 = plot.twinx()
        _ = lns3 = ax2.plot(t_m,Cs_m,linestyle=":", color=s,linewidth=3,label='Cs modelo') 
        _ = plot.set_xlabel('Tempo de cultivo (h)',weight='bold')               
        _ = plot.set_ylabel('Cx e Cp (g/L)', weight='bold')
    ...
    imprimir_perfil_concentracao_model_otim_exp(t_simul, Cx, Cs, Cp)
    
    def graf_cor (x,p,s): 
        tamanho_graf()
        f = plt.figure(figsize=(8.3,6), dpi = 54) 
        plot = f.add_subplot(111) 
        _ = lns1 = plot.plot(t_simul ,C_sim[:,0], color = x, linewidth=3,label='Cx modelo')
        _ = lns2 = plot.plot(t_simul,C_sim[:,2], linestyle="--", color=p,linewidth=3,label='Cp modelo')
        ax2 = plot.twinx()
        _ = lns3 = ax2.plot(t_simul,C_sim[:,1],linestyle=":", color=s,linewidth=3,label='Cs modelo') 
        _ = plot.set_xlabel('Tempo de cultivo (h)',weight='bold')               
        _ = plot.set_ylabel('Cx e Cp (g/L)', weight='bold')
        ax2.set_ylabel('Cs (g/L)', weight='bold') 
        lns = lns1+lns2+lns3
    ...
    if cont == 3:
        mult = -Kp_aiba*Cp
        mi = mimax*((Cs/(Ks+Cs))*np.exp(mult))
    if cont == 4:
        mi = mimax*((Cs**u)/(Ks+(Cs**u)))
    if cont == 5:
        mi = mimax*(Cs/(Ks+Cs))*(Kp_hh/(Kp_hh+Cp))
    if cont == 6:
        mi = mimax*(Cs/(Ks+Cs+(Cs*((Cs/Ke)**v))))
    if cont == 7:
        mi = mimax*((Cs/(Ks+Cs))*((1-(Cp/Cp_estr))**n))
    if cont == 8:
        mi = mimax*((Cs/(Ks+Cs))*((1-(Cx/Cx_estr))**m))
    if cont == 9:
        mi = np.repeat(mimax,len(t_simul))
    if (cont !=9):
        mi[mi<0] = 0
    ...
    sai_resul_simul.configure(text = "Simulacao_Concent_Produt_mi.xlsx", font = "arial 8 italic", fg = "black")
...
def aval_infl_mi_cin_const():
    # Vetor condição inicial:
    inic_cond_simul = [Cx0,t0]
    print(inic_cond_simul)
    # Vetor tempo:
    ...
    def batelconst(Cx,t):                   
        ...
        return(dCxdt) 
    ## Integração numérica:
    Cells1 = odeint(batelconst,inic_cond_simul,t_simul)
    # Segunda EDO:
    def batelconst(Cx,t):                   
        mi = mimax2 
        ...
        return(dCxdt)
    ## Integração numérica:
    Cells2 = odeint(batelconst,inic_cond_simul,t_simul)
    # Terceira EDO:
    def batelconst(Cx,t):                  
        mi = mimax3  
        ...
        return(dCxdt) 
    ## Integração numérica:
    Cells3 = odeint(batelconst,inic_cond_simul,t_simul)
    # Quarta EDO:
    def batelconst(Cx,t):                   
        mi = mimax4                           
        dCxdt = mi*Cx   
        ...
        return(dCxdt)
    ## Integração numérica:
    Cells4 = odeint(batelconst,inic_cond_simul,t_simul)
    # Quinta EDO:
    def batelconst(Cx,t):                   
        mi = mimax5                          
        dCxdt = mi*Cx   
        ...
        return(dCxdt)
    ## Integração numérica:
    Cells5 = odeint(batelconst,inic_cond_simul,t_simul)
    print(Cells5)
    ...
    tamanho_graf()
    plot = f.add_subplot(111)  
    _ = plt.plot(t_simul,Cells1[:,0],'m',linewidth=4,label= mimax1)    
    _ = plt.plot(t_simul,Cells2[:,0],'y',linewidth=4,label= mimax2)    
    _ = plt.plot(t_simul,Cells3[:,0],'b',linewidth=4,label= mimax3)    
    _ = plt.plot(t_simul,Cells4[:,0],'g',linewidth=4,label= mimax4)    
    _ = plt.plot(t_simul,Cells5[:,0],'c',linewidth=4,label= mimax5)  
    _ = plt.xlabel('Tempo de cultivo (h)',weight='bold')               
    _ = plt.ylabel('Concentração de células (g/L)', weight='bold')      
    _ = plt.legend(loc=0, title = "Valores de $\mu= (h^{-1}$)")                                         
    _ = plt.annotate(u'Perfil exponencial de\ncrescimento', xy=((max(t_simul))/2, (max(Cells5[:,0]))/3), xytext=((max(t_simul))/4, (max(Cells5[:,0]))/2), arrowprops=dict(facecolor='grey',shrink=0.05),size=15)   
    _ = plt.grid(True)                                                                                     
    f.patch.set_facecolor('white')                                           
    plt.style.use('default') 
    ...
    def salvar_concent():
        ...
        defaultextension='.png')
        plt.savefig(a)
    botao_com_graf(frame = frame44, comando_salvar = lambda : salvar_concent(), comando_destroy = canvas.get_tk_widget().destroy, x = 450, y = 130)
    ...
    def excel_influ_mi_cin_const():
        ...
        with pd.ExcelWriter('Concentracao_valores_mi.xlsx') as writer:
            ...
            writer.save()
        os.system("start EXCEL Concentracao_valores_mi.xlsx")
    sai_resul_simul.configure(text = "Concentracao_valores_mi.xlsx", font = "arial 8 italic", fg = "black")
     
    # Função botão acesso excel:
    def botao_excel(imagem, num_frame, x, y):
        load = Image.open(imagem)
        render = ImageTk.PhotoImage(load)
        img = Button(num_frame, image = render, border = 0, command = excel_influ_mi_cin_const)
        img.image = render
        img.place(x = x, y = y)
...
def explorer():
    ...
    def modelagem(cont_model):
        ...
        param_otim_alm = resultado_alm[0].round(4)
        ...
        if (cont_model == 0 or cont_model == 1):
            Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 943, y = 255)
            Label(frame2, text = param_otim_alm[1], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1021, y = 255)
        if (cont_model == 9):
                Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 943, y = 255)
        else:
            if cont_model >=2 and cont_model <=5:
                Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 943, y = 255)
                Label(frame2, text = param_otim_alm[1], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1021, y = 255)
                Label(frame2, text = param_otim_alm[6], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1099, y = 255)
            if (cont_model > 5):
                Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 943, y = 255)
                Label(frame2, text = param_otim_alm[1], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1021, y = 255)
                Label(frame2, text = param_otim_alm[6], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1099, y = 255)
                Label(frame2, text = param_otim_alm[7], font = "batang 11", justify = "center", fg = "black", bg = "grey75").place(x = 1163, y = 255)       
        if (cont_model == 9):
            Label(frame2, text = param_otim_alm[1] , font = "batang 11", fg = "black", bg = "grey75", justify = "center").place(x = 934, y = 417)
            Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "black", bg = "grey75", justify = "center").place(x = 1000, y = 417)
            Label(frame2, text = param_otim_alm[3] , font = "batang 11", fg = "black", bg = "grey75", justify = "center").place(x = 1090, y = 417)
            Label(frame2, text = param_otim_alm[4] , font = "batang 11", fg = "black", bg = "grey75", justify = "center").place(x = 1170, y = 417)
        else:
        ...
        cond_mimax_mi =  param_otim_alm[0] > 0.9 
        ...
        if (cont_model != 9):
            ...
            cond_yxs = param_otim_alm[2] >= C_exp[0,1]
            cond_mi_kd = param_otim_alm[1] > param_otim_alm[0]
        ...
        if (cond_mimax_mi or cond_Ks or cond_mi_kd or cond_param_neg == True):
            ...
            def buttonCallback(self):
                ...
                if button_flashing:
                    ...
                    flashColour(self, 0)
                    if cond_mimax_mi:
                        Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 943, y = 255)
                    if (cont_model != 9):
                        if cond_Ks:
                            Label(frame2, text = param_otim_alm[1], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 1021, y = 255)
                    if (cont_model != 9):
                        if cond_yxs:
                            Label(frame2, text = param_otim_alm[3] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1000, y = 417)
                    else:
                        if cond_yxs:
                            Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1000, y = 417)
                    if (cont_model != 9):
                        if cond_mi_kd:
                            Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 934, y = 417)
                    else:
                        if cond_mi_kd:
                            Label(frame2, text = param_otim_alm[1] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 934, y = 417)
                    if cond_param_neg == True:
                        if param_otim_alm[0] <= 0: #mimaximo, mi
                            Label(frame2, text = param_otim_alm[0], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 943, y = 255)
                        if (cont_model != 9):
                            if param_otim_alm[1] <= 0: #Ks
                                Label(frame2, text = param_otim_alm[1], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 1021, y = 255)
                        if (cont_model != 9):
                            if param_otim_alm[2] < 0 or cond_mi_kd or param_otim_alm[2] == -0.0: #Kd
                                Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 934, y = 417)
                        else:
                            if param_otim_alm[1] < 0 or cond_mi_kd or param_otim_alm[1] == -0.0: #Kd
                                Label(frame2, text = param_otim_alm[1] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 934, y = 417)
                        if (cont_model != 9):
                            if (param_otim_alm[3] <= 0 or cond_yxs): #Yxs
                                Label(frame2, text = param_otim_alm[3] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1000, y = 417)
                        else: 
                            if (param_otim_alm[2] <= 0 or cond_yxs): #Yxs
                                Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1000, y = 417)
                        if (cont_model != 9):
                            if param_otim_alm[4] <= 0: #alfa
                                Label(frame2, text = param_otim_alm[4] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1090, y = 417)
                        else:
                            if param_otim_alm[2] <= 0: #alfa
                                Label(frame2, text = param_otim_alm[2] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1090, y = 417)
                        if (cont_model != 9):
                            if param_otim_alm[5] < 0: #beta
                                Label(frame2, text = param_otim_alm[5] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1170, y = 417)
                        else:
                            if param_otim_alm[4] < 0: #beta
                                Label(frame2, text = param_otim_alm[4] , font = "batang 11", fg = "red4", bg = "grey75", justify = "center").place(x = 1170, y = 417)
                        if cont_model == 2:
                            if param_otim_alm[6] > 55:
                                Label(frame2, text = param_otim_alm[6], font = "batang 11", justify = "center", fg = "red4", bg = "gre75y").place(x = 1099, y = 255)
                        if cont_model >= 3 and cont_model <=5 :
                            if (param_otim_alm[6] <= 0 or param_otim_alm[6] > 10):
                                Label(frame2, text = param_otim_alm[6], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 1099, y = 255)
                        if cont_model >= 6 and cont_model <=8:
                            if param_otim_alm[6] <= 0:
                                Label(frame2, text = param_otim_alm[6], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 1142, y = 255)
                            if param_otim_alm[7] <= 0:
                                Label(frame2, text = param_otim_alm[7], font = "batang 11", justify = "center", fg = "red4", bg = "grey75").place(x = 1163, y = 255)
                else:
        ...
        def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
            ...
            _ = lns3 = plot.plot(t_m,Cp_m, linestyle="--", color=p,linewidth=3,label='Cp modelo')  
            _ = lns4 = plot.plot(t_ajus ,Cp_ajus,'^',color = p, markersize=6,label='Cp experimental')
            ax2 = plot.twinx()
            _ = lns5 = ax2.plot(t_m,Cs_m,linestyle=":", color=s,linewidth=3,label='Cs modelo') 
            _ = lns6 = ax2.plot(t_ajus,Cs_ajus,'s',color=s, markersize=6,label='Cs experimental')
            _ = plot.set_xlabel('Tempo de cultivo (h)',weight='bold')               
            _ = plot.set_ylabel('Cx e Cp (g/L)', weight='bold')
        ...
        def graf_cor (x,p,s): 
            ...
            f = plt.figure(figsize=(8.3,6), dpi = 54) 
            plot = f.add_subplot(111) 
            _ = lns1 = plot.plot(t ,C_otim[:,0], color = x, linewidth=3,label='Cx modelo')
            _ = lns2 = plot.plot(t_exp ,C_exp[:,0],'o',color=x,markersize=6, label='Cx experimental')
            _ = lns3 = plot.plot(t,C_otim[:,2], linestyle="--", color=p,linewidth=3,label='Cp modelo')  
            _ = lns4 = plot.plot(t_exp ,C_exp[:,2],'^',color = p, markersize=6,label='Cp experimental')
            ax2 = plot.twinx()
            _ = lns5 = ax2.plot(t,C_otim[:,1],linestyle=":", color=s,linewidth=3,label='Cs modelo') 
            _ = lns6 = ax2.plot(t_exp,C_exp[:,1],'s',color=s, markersize=6,label='Cs experimental')
            _ = plot.set_xlabel('Tempo de cultivo (h)',weight='bold')               
            _ = plot.set_ylabel('Cx e Cp (g/L)', weight='bold')
            _ = ax2.set_ylabel('Cs (g/L)', weight='bold') 
            lns = lns1+lns2+lns3+lns4+lns5+lns6
       ...
       ## Escolha das cores:
        ...
        if cont_model == 9:
            mi_exp = np.repeat(param_otim_alm[0],len(t_exp))
            mi = np.repeat(param_otim_alm[0],len(t))
        mi_exp[mi_exp<0] = 0
        mi[mi<0] = 0
        ...
        def excel_param():
            ...
            if cont_model == 8: #Lee
                ...
                df_params = pd.concat([df_params_model, df_params_IC], axis = 1)
            if cont_model == 9: #mi constante
                df_params_model =pd.DataFrame({'mi(h-¹)':[param_otim_alm[0]],'Kd(h-¹)':[param_otim_alm[1]],'Yxs(gcél/gsub)':[param_otim_alm[2]], 'alfa(gprod/gcél)': [param_otim_alm[3]], 'beta(gprod/gcél.h)':[param_otim_alm[4]]})
                df_params_IC = pd.DataFrame({'IC mi(h-¹)':[ICpar[0]],'IC Kd(h-¹)':[ICpar[1]],'IC Yxs(gcél/gsub)':[ICpar[2]], 'IC alfa(gprod/gcél)': [ICpar[3]], 'IC beta(gprod/gcél.h)':[ICpar[4]]})
                df_params = pd.concat([df_params_model, df_params_IC], axis = 1)
    ...
    def print_me_2():
        ...
        if value_2 == "AUSÊNCIA DE INIBIÇÃO":
            ...
            Button(frame45, text = "MODELAR", bg = "gray20", fg="white", borderwidth=2, relief="ridge", font="batang 11", width = 8, command = click_mi_const).place(x = 112, y = 28)
...
def capt_val_esc_mi_const():
    ...
    simulacao(cont = 9)
def capt_val_esc_infl_mi_const():
    global Cx0, t0, tf, mimax1, mimax2, mimax3, mimax4, mimax5
    Cx0 = float(entr_Cx0_infl_mi_const.get())
    t0 = float(entr_t0_infl_mi_const.get())
    tf = float(entr_tf_infl_mi_const.get())
    mimax1 = float(slider_mimax1_infl_mi_const.get())
    mimax2 = float(slider_mimax2_infl_mi_const.get())
    mimax3 = float(slider_mimax3_infl_mi_const.get())
    mimax4 = float(slider_mimax4_infl_mi_const.get())
    mimax5 = float(slider_mimax5_infl_mi_const.get())
    print(Cx0, t0, tf, mimax1, mimax2, mimax3, mimax4, mimax5)
    aval_infl_mi_cin_const()
...
def entr_simul_mimax1_infl_mi_const(frame):
    ## Criação dos botões deslizáveis:
    input = tk.DoubleVar(value=0.01)
    global spin_mimax1_infl_mi_const, slider_mimax1_infl_mi_const
    spin_mimax1_infl_mi_const = tk.Spinbox(frame, textvariable=input, wrap=True, width=4)
    spin_mimax1_infl_mi_const.place(x = 15, y = 37)
    slider_mimax1_infl_mi_const = ttk.Scale(frame, variable=input, from_= 0.01, to = 1.5, orient='horizontal',length = 60)
    slider_mimax1_infl_mi_const.place(x = 52, y = 32) 
def entr_simul_mimax2_infl_mi_const(frame):
    ## Criação dos botões deslizáveis:
    input = tk.DoubleVar(value=0.01)
    global spin_mimax2_infl_mi_const, slider_mimax2_infl_mi_const
    spin_mimax2_infl_mi_const = tk.Spinbox(frame, textvariable=input, wrap=True, width=4)
    spin_mimax2_infl_mi_const.place(x = 15, y = 88)
    slider_mimax2_infl_mi_const = ttk.Scale(frame, variable=input, from_= 0.01, to = 1.5, orient='horizontal',length = 60)
    slider_mimax2_infl_mi_const.place(x = 52, y = 83) 
def entr_simul_mimax3_infl_mi_const(frame):
    ## Criação dos botões deslizáveis:
    input = tk.DoubleVar(value=0.01)
    global spin_mimax3_infl_mi_const, slider_mimax3_infl_mi_const
    spin_mimax3_infl_mi_const = tk.Spinbox(frame, textvariable=input, wrap=True, width=4)
    spin_mimax3_infl_mi_const.place(x = 15, y = 139)
    slider_mimax3_infl_mi_const = ttk.Scale(frame, variable=input, from_= 0.01, to = 1.5, orient='horizontal',length = 60)
    slider_mimax3_infl_mi_const.place(x = 52, y = 134) 
def entr_simul_mimax4_infl_mi_const(frame):
    ## Criação dos botões deslizáveis:
    input = tk.DoubleVar(value=0.01)
    global spin_mimax4_infl_mi_const, slider_mimax4_infl_mi_const
    spin_mimax4_infl_mi_const = tk.Spinbox(frame, textvariable=input, wrap=True, width=4)
    spin_mimax4_infl_mi_const.place(x = 15, y = 190)
    slider_mimax4_infl_mi_const = ttk.Scale(frame, variable=input, from_= 0.01, to = 1.5, orient='horizontal',length = 60)
    slider_mimax4_infl_mi_const.place(x = 52, y = 185) 
def entr_simul_mimax5_infl_mi_const(frame):
    ## Criação dos botões deslizáveis:
    input = tk.DoubleVar(value=0.01)
    global spin_mimax5_infl_mi_const, slider_mimax5_infl_mi_const
    spin_mimax5_infl_mi_const = tk.Spinbox(frame, textvariable=input, wrap=True, width=4)
    spin_mimax5_infl_mi_const.place(x = 120, y = 190)
    slider_mimax5_infl_mi_const = ttk.Scale(frame, variable=input, from_= 0.01, to = 1.5, orient='horizontal',length = 60)
    slider_mimax5_infl_mi_const.place(x = 157, y = 185)       
...
def entr_infl_mi_const(frame):
    global entr_Cx0_infl_mi_const, entr_Cs0_infl_mi_const, entr_Cp0_infl_mi_const, entr_t0_infl_mi_const, entr_tf_infl_mi_const
    entr_Cx0_infl_mi_const = tk.Entry(frame, width = 7, borderwidth = 2, relief = "sunken", font = "batang 8 bold", bg = "white", fg = "black")
    entr_Cx0_infl_mi_const.place(x = 50, y = 257)
    entr_t0_infl_mi_const = tk.Entry(frame, width = 7, borderwidth = 2, relief = "sunken", font = "batang 8 bold", bg = "white", fg = "black")
    entr_t0_infl_mi_const.place(x = 50, y = 287)
    entr_tf_infl_mi_const = tk.Entry(frame, width = 7, borderwidth = 2, relief = "sunken", font = "batang 8 bold", bg = "white", fg = "black")
    entr_tf_infl_mi_const.place(x = 140, y = 287)
...
## Influência valor da taxa mi na cinética constante:
def infl_mi_constante():
    separ_simul_infl_mi_const(frame = frame43)
    labels_saida_mi_const(frame43)
    labels(frame = frame43, texto = u"\u03bc1 (h\u207b\u00b9)", fonte = "times 9 bold", borda = "sunken", x = 2, y = 14)
    labels(frame = frame43, texto = u"\u03bc2 (h\u207b\u00b9)", fonte = "times 9 bold", borda = "sunken", x = 2, y = 65)
    labels(frame = frame43, texto = u"\u03bc3 (h\u207b\u00b9)", fonte = "times 9 bold", borda = "sunken", x = 2, y = 116)
    labels(frame = frame43, texto = u"\u03bc4 (h\u207b\u00b9)", fonte = "times 9 bold", borda = "sunken", x = 2, y = 167)
    labels(frame = frame43, texto = u"\u03bc5 (h\u207b\u00b9)", fonte = "times 9 bold", borda = "sunken", x = 102, y = 167)
    entr_simul_mimax1_infl_mi_const(frame43)
    entr_simul_mimax2_infl_mi_const(frame43)
    entr_simul_mimax3_infl_mi_const(frame43)
    entr_simul_mimax4_infl_mi_const(frame43)
    entr_simul_mimax5_infl_mi_const(frame43)
    entr_infl_mi_const(frame43)
    Button(frame43, text = "Simular", font = "batang 8 bold", fg = "white", bg = "black", borderwidth = 2, relief = "sunken", command = capt_val_esc_infl_mi_const).place(x = 240, y = 282)
...
def print_me_1():
        value_1 = combo_1.get()
        print(value_1)
        # Análise para criação do notebook para plotagem gráfica:
        if value_1 == "CINÉTICA CONSTANTE SEM INIBIÇÃO":
            notebook_graf_model_infl_mi_const()
        else:
            notebook_graf_simul()
