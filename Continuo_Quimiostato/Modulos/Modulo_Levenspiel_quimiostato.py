# -*- coding: utf-8 -*-
"""
Created on Wed Dec 23 15:06:21 2020

@author: Bruna Aparecida
"""


                    # MODELO CINÉTICO DE CRESCIMENTO MICROBIANO INIBIÇÃO PRODUTO - LEVENSPIEL #
...
def entr_Levenspiel():
    def entr_val_Levenspiel():
        ...
        Q = 0.2
        t0 = 20
        return(mimaximo, Ks, Kd, Cs0_alim, tf, Yxs, alfa, beta, n, Cp_estr, V, Q, t0)
    mimaximo, Ks, Kd, Cs0_alim, tf, Yxs, alfa, beta, n, Cp_estr, V, Q, t0 = entr_val_Levenspiel()
    entr_val_params = [mimaximo, Ks, Kd, Cs0_alim, tf, Yxs, alfa, beta, n, Cp_estr]
    t = np.arange(t0,tf,0.5)
    entr_val_oper = [V, Q, t0]
    return(entr_val_params, t, entr_val_oper)

# Função 2)
def func_quimiostato_Levenspiel_simul():
    params = entr_Levenspiel()[0]
    oper = entr_Levenspiel()[2]
    mimax = params[0]
    Ks = params[1]
    Yxs = params[5]
    alfa = params[6]
    ...
    Cs = D
    Cx = (Cs0_alim - Cs) * Yxs
    mi = mimax * ((Cs / (Ks + Cs)) * ((abs(1- (Cp / Cp_estr))) ** n))
...
# Função 3)
def func_args_quimiostato_Levenspiel():
    def func_quimiostato_Levenspiel_modelag(C, t, *args):
        params = entr_Levenspiel()[0]
        oper = entr_Levenspiel()[2]
        mimax = args[0]
        Ks = args[1]
        ...
        Cx_pont_lav = (Cs0_alim - Cs_pont_lav) * Yxs
        return(D, Cs, Cx, mi, Cp, D_pont_lav, Cs_pont_lav, Cx_pont_lav)
    return(func_args_quimiostato_Levenspiel)
