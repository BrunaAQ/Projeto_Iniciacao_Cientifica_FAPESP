# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 11:16:08 2020

@author: Bruna Aparecida
"""

                         ## MODELAGEM BATELADA ALIMENTADA VAZÃO LINEAR CONSTANTE CINÉTICA CLÁSSICA ##

# Importação das bibliotecas necessárias para as partes não modulares:
import Modulos_Monod_bat_alim_sem_Kd
...
dad_entr_geral = Modulos_Monod_bat_alim_sem_Kd.entr_Monod()
...
def bat_Monod (Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Monod, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_Monod (Concent,t_exp_alim):
    Cx,Cs,Cp=Concent
    mi = mimaximo*(Cs/(Ks+Cs))
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Monod, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Monod_bat_alim_sem_Kd.modelag_bat_Monod_func_args()
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Monod, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Monod, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("alfa_bat:",resultado_alm_bat[0][3],"+/-",ICpar_bat[0],"(gp/gx)")
print("beta_bat:",resultado_alm_bat[0][3],"+/-",ICpar_bat[0],"[gp/(gx.h)]")
#*ETAPA 2*#
...
print("beta_alim:",resultado_alm_alim[0][3],"+/-",ICpar_alim[0],"[gp/(gx.h)]")
...
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [pars_entr[0]],'KSX_sim(gp/gs)': [pars_entr[1]],
                              ...
                              'alfa(gprod/gcél)': [pars_entr[8]], 'beta_sim(gprod/gcél.h)': [pars_entr[9]],
                              "Q(L.h)": [param_oper_alim[4]], "V0(L)": [param_oper_alim[2]], "tf_batelada(h)": [pars_entr[6]], 
                              "Cs0_alim(gs/L)": [param_oper_alim[0]], "a(-)": [param_oper_alim[6]]})
df_saida_monod = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Monod_alim_lin.xlsx') as writer:
    df_saida_monod.to_excel(writer, sheet_name="Saída_Monod_alim_lin")
    writer.save()
...
mi_exp = mimaximo_otim*(Cs_exp/(Ks_otim + Cs_exp))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*(Cs/(Ks_otim + Cs))
mi[mi<0] = 0
...
# Gráficos para a variação temporal da vazão e do volume:

#Variação do volume reacional em função do tempo:
##Função V(t) integrada:
V_calc = (Q0*(t_exp_alim+(a*t_exp_alim**2)))+V0

#Gráfico variação de volume x tempo:
tam_graf()
_ = f = plt.figure() 
_ = plt.plot(t_exp_alim,V_calc,'brown',linewidth=3, label = "Volume")
...
##Função Q(t) original:
Q_calc = Q0*(1+a*t_exp_alim)

# Plotando figura gráfica - VAZÃO:
tam_graf()
_ = f = plt.figure() 
_ = plt.plot(t_exp_alim,Q_calc,'magenta',linewidth=3, label = "Vazão")
_ = plt.xlabel("Tempo de cultivo alimentado (h)",weight='bold')
