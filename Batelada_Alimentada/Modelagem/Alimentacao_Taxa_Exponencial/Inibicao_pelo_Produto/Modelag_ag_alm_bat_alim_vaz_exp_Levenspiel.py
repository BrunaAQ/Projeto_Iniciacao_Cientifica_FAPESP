# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 14:36:22 2020

@author: Bruna Aparecida
"""

                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE LEVENSPIEL ##
...
dad_entr_geral = Modulos_Levenspiel_bat_alim.entr_Levenspiel()
...
def bat_Levenspiel(Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Levenspiel, cond_inic_bat, t_exp_bat)
...
beta_exp = param_oper_alim[8]

## Integração numérica (sistema de EDOs):
def bat_alim_Levenspiel(Concent,t_exp_alim):
    Cx,Cs,Cp = Concent
    mi = mimaximo*((Cs/(Ks+Cs))*((abs(1-(Cp/Cp_estr)))**n))
    multiplicacao = beta_exp*t_exp_alim
    exponencial = np.exp(multiplicacao)
    D = (Q0*np.exp(beta_exp*t_exp_alim))/(((Q0/beta_exp)*(exponencial - 1)) + V0)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Levenspiel, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Levenspiel_bat_alim.modelag_bat_Levenspiel_func_args()
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Levenspiel, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Levenspiel, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("n_bat:",resultado_alm_bat[0][5])#,"+/-",ICpar_bat[5],"[adimensional]")
print("Cp_estr_bat:",resultado_alm_bat[0][6])#,"+/-",ICpar_bat[5],"[gp/l]")
#*ETAPA 2*#
...
print("n_alim:",resultado_alm_alim[0][5])#"+/-",ICpar_bat[0],"[adimensional]")
print("Cp_estr_bat:",resultado_alm_bat[0][6])#,"+/-",ICpar_bat[5],"[gp/l]")
...
Ttotal_exp = np.arange(0,param_oper_alim[1],1.5)
divisor = len(Cx)
...
n_otim = resultado_alm_alim[0][5]
Cp_estr_otim = resultado_alm_alim[0][6]

mi_exp = mimaximo_otim*((Cs_exp/(Ks_otim + Cs_exp))*((abs(1-(Cp_exp/Cp_estr_otim)))**n_otim))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*((Cs/(Ks_otim + Cs))*((abs(1-(Cp/Cp_estr_otim)))**n_otim))
mi[mi<0] = 0
