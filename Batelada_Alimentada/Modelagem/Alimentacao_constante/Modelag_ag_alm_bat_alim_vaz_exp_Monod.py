# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 14:25:10 2020
...
Q = param_oper_alim[4]
Q0 = param_oper_alim[7]
beta = param_oper_alim[8]

## Integração numérica (sistema de EDOs):
def bat_alim_Monod (Concent,t_exp_alim):
    Cx,Cs,Cp=Concent
    mi = mimaximo*(Cs/(Ks+Cs))
    multiplicacao = beta*t_exp_alim
    exponencial = np.exp(multiplicacao)
    D = (Q0*np.exp(beta*t_exp_alim))/(((Q0/beta)*(exponencial - 1)) + V0)
    dCxdt = (mi-D)*Cx
...
V_calc = ((Q0/beta)*((np.exp(beta*t_exp_alim)) - 1)) + V0
...
##Função Q(t) original:
Q_calc = Q0*np.exp(beta*t_exp_alim)
