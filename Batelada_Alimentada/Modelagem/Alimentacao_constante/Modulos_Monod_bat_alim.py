...
def entr_Monod():
    def entr_val_Monod():
        ...
        Ks = 2 #3.14 #unidade g/L - constante de semi-saturação
        ...
        Vf = 6
        Q = 0.252
        return(mimaximo,Ks,Kd,Cx0_bat,Cs0_bat,Cp0_bat,tf_bat,Yxs,alfa,beta, Cs0_alim, tf_alim, V0, Vf, Q)
    mimaximo, Ks, Kd, Cx0_bat, Cs0_bat, Cp0_bat, tf_bat, Yxs, alfa, beta, Cs0_alim, tf_alim, V0, Vf, Q = entr_val_Monod()
    ...
    entr_val_alim = Cs0_alim, tf_alim, V0, Vf, Q
...
# Função 4)
def modelag_bat_alim_Monod_func_args():
    def bat_alim_Monod(C, t, *args):
        mimaximo = args[0]
        Ks = args[1]
        Yxs = args[2]
        alfa = args[3]
        ...
        D=Q/(V0+Q*tal)
        dCxdt=(mi-D)*C[0]
        dCsdt=D*(Cs0_alim-C[1])-((mi*C[0])/Yxs)
        dCpdt=D*(Cp0_alim-Cp)+C[0]*(beta+alfa*mi)
        return(dCxdt,dCsdt,dCpdt)
    return(bat_alim_Monod)
