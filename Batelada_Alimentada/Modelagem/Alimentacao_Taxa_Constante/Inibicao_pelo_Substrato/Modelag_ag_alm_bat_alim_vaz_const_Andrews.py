# -*- coding: utf-8 -*-
"""
Created on Fri Nov  6 10:22:07 2020

@author: Bruna Aparecida
"""

                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE ANDREWS ##
...
dad_entr_geral = Modulos_Andrews_bat_alim.entr_Andrews()
...
def bat_Andrews(Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Andrews, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_Andrews(Concent,t_exp_alim):
    Cx,Cs,Cp = Concent
    mi = mimaximo*(Cs/(Ks + Cs + ((Cs**2)/KIS)))
    D = Q/(V0+Q*t_exp_alim)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Andrews, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Andrews_bat_alim.modelag_bat_Andrews_func_args()
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Andrews, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Andrews, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("KIS_bat:",resultado_alm_bat[0][5])#,"+/-",ICpar_bat[5],"[gp/(gx.h)]")
#*ETAPA 2*#
...
print("KIS_alim:",resultado_alm_alim[0][5])#"+/-",ICpar_bat[0],"[gp/(gx.h)]")
...
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [pars_entr[0]],'KSX_sim(gp/gs)': [pars_entr[1]],
                              ...
                              'alfa(gprod/gcél)': [pars_entr[8]], 'beta_sim(gprod/gcél.h)': [pars_entr[9]], "KIS(g/L)": [pars_entr[10]],
                              "Q(L.h)": [param_oper_alim[4]], "V0(L)": [param_oper_alim[2]], "tf_batelada(h)": [pars_entr[6]], 
                              "Cs0_alim(gs/L)": [param_oper_alim[0]]})
df_saida_andrews = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Andrews_alim_const.xlsx') as writer:
    df_saida_andrews.to_excel(writer, sheet_name="Saída_Contois_alim_const")
    writer.save()
...
KIS_otim = resultado_alm_alim[0][5]

mi_exp = mimaximo_otim*(Cs_exp/(Ks_otim + Cs_exp + ((Cs_exp**2)/KIS_otim)))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*(Cs/(Ks_otim + Cs + ((Cs**2)/KIS_otim)))
mi[mi<0] = 0
