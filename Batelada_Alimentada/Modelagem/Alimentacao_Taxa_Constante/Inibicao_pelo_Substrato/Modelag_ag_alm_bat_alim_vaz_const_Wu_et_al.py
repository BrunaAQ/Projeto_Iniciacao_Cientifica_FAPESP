# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 10:27:16 2020

@author: Bruna Aparecida
"""

                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE WU ET AL ##
...
dad_entr_geral = Modulos_Wu_et_al_bat_alim.entr_Wu_et_al()
...
def bat_Wu_et_al(Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Wu_et_al, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_Wu_et_al(Concent,t_exp_alim):
    Cx,Cs,Cp = Concent
    mi = mimaximo * (Cs/(Ks + Cs + Cs*((Cs/KE)**v)))
    D = Q/(V0+Q*t_exp_alim)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Wu_et_al, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Wu_et_al_bat_alim.modelag_bat_Wu_et_al_func_args()
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Wu_et_al, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Wu_et_al, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("KE_bat:",resultado_alm_bat[0][5])#,"+/-",ICpar_bat[5],"[g/l]")
print("v_bat:",resultado_alm_bat[0][6])#,"+/-",ICpar_bat[6],"[adimensional]")
#*ETAPA 2*#
...
print("KE_alim:",resultado_alm_alim[0][5])#"+/-",ICpar_bat[5],"[g/l]")
print("v_bat:",resultado_alm_bat[0][6])#,"+/-",ICpar_bat[6],"[adimensional]")
...
df_saida_wu = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Wu_et_al_alim_const.xlsx') as writer:
    df_saida_wu.to_excel(writer, sheet_name="Saída_Wu_et_al_alim_const")
    writer.save()
'''   
def tam_graf():
...
KE_otim = resultado_alm_alim[0][5]
v_otim = resultado_alm_alim[0][6]

mi_exp = mimaximo_otim * (Cs_exp/(Ks_otim + Cs_exp + Cs_exp*((Cs_exp/KE_otim)**v_otim)))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim * (Cs/(Ks_otim + Cs + Cs*((Cs/KE_otim)**v_otim)))
mi[mi<0] = 0
