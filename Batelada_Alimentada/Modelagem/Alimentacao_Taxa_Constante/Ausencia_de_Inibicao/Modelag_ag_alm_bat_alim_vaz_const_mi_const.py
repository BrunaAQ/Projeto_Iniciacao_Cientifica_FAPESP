# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 14:35:51 2020

@author: Bruna Aparecida
"""
                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA CONSTANTE ##

# Importação das bibliotecas necessárias para as partes não modulares:
import Modulos_mi_const_bat_alim
...
dad_entr_geral = Modulos_mi_const_bat_alim.entr_mi_const()
...
def bat_mi_const (Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
    dCxdt = mi*Cx
    dCsdt = (-1/Yxs)*mi*Cx
...
### Matriz de retorno:
C_exp_bat = odeint(bat_mi_const, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_mi_const (Concent,t_exp_alim):
    Cx,Cs,Cp=Concent
    D = Q/(V0+Q*t_exp_alim)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_mi_const, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_mi_const_bat_alim.modelag_bat_mi_const_func_args()
...
limites_mi_const = Modulo_peso_limite_AG.limites()[9]
# Definição dos argumentos:
args = (t_exp_bat,C_exp_bat)
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_mi_const, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_mi_const , args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
ContadorCs_exp = 0
for ContadorCs_exp in range(len(Ttotal_exp)):
    if (Cs_exp[ContadorCs_exp] < 0):
        Cs_exp[ContadorCs_exp] = 0
        Cx_exp[ContadorCs_exp] = Cx_exp[ContadorCs_exp - 1]
        Cp_exp[ContadorCs_exp] = Cp_exp[ContadorCs_exp - 1]

'''
ContadorCs = 0
for ContadorCs in range(len(Ttotal)):
    if (Cs[ContadorCs] < 0):
        Cs[ContadorCs] = 0
...
mi_otim = resultado_alm_alim[0][0]

mi_exp = np.repeat(mi_otim, len(Ttotal_exp))
mi_exp[mi_exp<0] = 0
mi = np.repeat(mi_otim, len(Ttotal))
mi[mi<0] = 0
