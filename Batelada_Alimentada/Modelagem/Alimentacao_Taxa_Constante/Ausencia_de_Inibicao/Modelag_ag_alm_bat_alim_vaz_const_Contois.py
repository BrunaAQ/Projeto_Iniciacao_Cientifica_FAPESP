# -*- coding: utf-8 -*-
"""
Created on Fri Nov  6 08:54:37 2020

@author: Bruna Aparecida
"""


                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE CONTOIS ##
...
dad_entr_geral = Modulos_Contois_bat_alim.entr_Contois()
...
def bat_Contois (Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Contois, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_Contois(Concent,t_exp_alim):
    Cx,Cs,Cp=Concent
    mi = mimaximo*(Cs/((KSX*Cx) + Cs))
    D = Q/(V0+Q*t_exp_alim)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Contois, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Contois_bat_alim.modelag_bat_Contois_func_args()
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Contois, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Contois, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("KSX_bat:",resultado_alm_bat[0][1],"+/-",ICpar_bat[1],"(g/l)")
print("Yxs_bat:",resultado_alm_bat[0][2],"+/-",ICpar_bat[2],"(gx/gs)")
print("alfa_bat:",resultado_alm_bat[0][3],"+/-",ICpar_bat[0],"(gp/gx)")
print("beta_bat:",resultado_alm_bat[0][4],"+/-",ICpar_bat[0],"[gp/(gx.h)]")
#*ETAPA 2*#
...
print("KSX_alim:",resultado_alm_alim[0][1],"+/-",ICpar_alim[1],"(g/l)")
...
print(len(Cx_exp))
print(len(t_bat), len(t_alim), len(Cx))
...
df_saida_contois = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Contois_alim_const.xlsx') as writer:
    df_saida_contois.to_excel(writer, sheet_name="Saída_Contois_alim_const")
    writer.save()
