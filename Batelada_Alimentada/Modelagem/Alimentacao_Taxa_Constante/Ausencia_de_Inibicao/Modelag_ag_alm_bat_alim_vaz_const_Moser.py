# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 11:36:14 2020
...
def bat_Moser (Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Moser, cond_inic_bat, t_exp_bat)
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Moser, cond_inic_alim, t_exp_alim)
...
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Moser, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Moser, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("u_bat:",resultado_alm_bat[0][5],"+/-",ICpar_bat[0],"[adimensional]")
#*ETAPA 2*#
...
print("u_alim:",resultado_alm_alim[0][5],"+/-",ICpar_alim[0],"[adimensional]")
...
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [pars_entr[0]],'KSX_sim(gp/gs)': [pars_entr[1]],
                              ...
                              'alfa(gprod/gcél)': [pars_entr[8]], 'beta_sim(gprod/gcél.h)': [pars_entr[9]], "u(-)": [pars_entr[10]],
                              "Q(L.h)": [param_oper_alim[4]], "V0(L)": [param_oper_alim[2]], "tf_batelada(h)": [pars_entr[6]], 
                              "Cs0_alim(gs/L)": [param_oper_alim[0]]})
df_saida_moser = pd.concat([df_concents, df_params], axis=1 )
with pd.ExcelWriter('Sim_Moser_alim_const.xlsx') as writer:
    df_saida_moser.to_excel(writer, sheet_name="Saída_Moser_alim_const")
    writer.save()
...
u_otim = resultado_alm_alim[0][5]

mi_exp = mimaximo_otim*(((Cs_exp)**u_otim)/(Ks_otim+((Cs_exp)**u_otim)))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*(((Cs)**u_otim)/(Ks_otim+((Cs)**u_otim)))
mi[mi<0] = 0
