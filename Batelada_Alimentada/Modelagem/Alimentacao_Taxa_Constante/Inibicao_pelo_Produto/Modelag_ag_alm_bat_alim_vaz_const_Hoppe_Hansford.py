# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 17:33:01 2020

@author: Bruna Aparecida
"""

                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE HOPPE & HANSFORD ##
...
dad_entr_geral = Modulos_Hoppe_Hansford_bat_alim.entr_Hoppe_Hansford()
...
def bat_Hoppe_Hansford(Concent,t_exp_bat):
    Cx,Cs,Cp = Concent
...
### Matriz de retorno:
C_exp_bat = odeint(bat_Hoppe_Hansford, cond_inic_bat, t_exp_bat)
...
## Integração numérica (sistema de EDOs):
def bat_alim_Hoppe_Hansford(Concent,t_exp_alim):
    Cx,Cs,Cp = Concent
    mi = mimaximo*(Cs/(Ks+Cs))*(Kp/(Kp+Cp))
    D = Q/(V0+Q*t_exp_alim)
    dCxdt = (mi-D)*Cx
...
### Matriz de retorno:
C_exp_alim = odeint(bat_alim_Hoppe_Hansford, cond_inic_alim, t_exp_alim)
...
func_args_bat = Modulos_Hoppe_Hansford_bat_alim.modelag_bat_Hoppe_Hansford_func_args()
...
limites_Hoppe_Hansford = Modulo_peso_limite_AG.limites()[4]
# Definição dos argumentos:
args = (t_exp_bat,C_exp_bat)
resultado_ag_bat = differential_evolution(func_obj_ag_bat, limites_Hoppe_Hansford, args=args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
resultado_ag_alim = differential_evolution(func_obj_ag_alim, limites_Hoppe_Hansford, args = args, popsize=5,  tol=0.01, mutation=(0.5, 1), recombination=0.7, updating='immediate')
...
print("Kp_bat:",resultado_alm_bat[0][5])#,"+/-",ICpar_bat[5],"[g/l]")
#*ETAPA 2*#
...
print("Kp_alim:",resultado_alm_alim[0][5])#"+/-",ICpar_bat[0],"[g/l]")
...
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [pars_entr[0]],'KSX_sim(gp/gs)': [pars_entr[1]],
                              ...
                              'alfa(gprod/gcél)': [pars_entr[8]], 'beta_sim(gprod/gcél.h)': [pars_entr[9]], "Kp(g/L)": [pars_entr[10]],
                              "Q(L.h)": [param_oper_alim[4]], "V0(L)": [param_oper_alim[2]], "tf_batelada(h)": [pars_entr[6]], 
                              "Cs0_alim(gs/L)": [param_oper_alim[0]]})
df_saida_hoppe_hansford = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Hoppe_Hansford_alim_const.xlsx') as writer:
    df_saida_hoppe_hansford.to_excel(writer, sheet_name="Saída_Hoppe_Hansford_alim_const")
    writer.save()
...
mi_exp = mimaximo_otim*(Cs_exp/(Ks_otim + Cs_exp))*(Kp_otim/(Kp_otim + Cp_exp))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*(Cs/(Ks_otim + Cs))*(Kp_otim/(Kp_otim + Cp))
mi[mi<0] = 0
