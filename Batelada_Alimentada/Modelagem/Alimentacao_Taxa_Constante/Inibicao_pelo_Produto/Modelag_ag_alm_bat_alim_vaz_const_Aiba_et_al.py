# -*- coding: utf-8 -*-
"""
Created on Tue Nov 10 20:23:22 2020
...
"""
                    ...
                    ## MODELAGEM BATELADA ALIMENTADA À VAZÃO CONSTANTE CINÉTICA DE AIBA ET AL ##
...
print("Kp_alim:",resultado_alm_alim[0][5])#"+/-",ICpar_bat[0],"[l/(g)]")
...
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [pars_entr[0]],'KSX_sim(gp/gs)': [pars_entr[1]],
                              ...
                              'alfa(gprod/gcél)': [pars_entr[8]], 'beta_sim(gprod/gcél.h)': [pars_entr[9]], "Kp(L/g)": [pars_entr[10]],
                              "Q(L.h)": [param_oper_alim[4]], "V0(L)": [param_oper_alim[2]], "tf_batelada(h)": [pars_entr[6]], 
                              "Cs0_alim(gs/L)": [param_oper_alim[0]]})
df_saida_aiba = pd.concat([df_concents, df_params], axis=1)
with pd.ExcelWriter('Sim_Aiba_et_al_alim_const.xlsx') as writer:
    df_saida_aiba.to_excel(writer, sheet_name="Saída_Aiba_et_al_alim_const")
    writer.save()
