# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 11:28:52 2020

@author: Bruna Aparecida
"""

                                  # MODELO CINÉTICO DE CRESCIMENTO MICROBIANO SEM INIBIÇÃO - mi constante #
...
# Função 1)
def entr_mi_const():
    def entr_val_mi_const():
        mi = 0.4 #0.45 #unidade 1/h - taxa específica de crescimento
        ...
        Q0_exp = 0.252
        beta = 0.1
        return(mi,Kd,Cx0_bat,Cs0_bat,Cp0_bat,tf_bat,Yxs,alfa,beta, Cs0_alim, tf_alim, V0, Vf, Q, Q0, a, Q0_exp, beta)
    mi, Kd, Cx0_bat, Cs0_bat, Cp0_bat, tf_bat, Yxs, alfa, beta, Cs0_alim, tf_alim, V0, Vf, Q, Q0, a, Q0_exp, beta = entr_val_mi_const()
    entr_val_bat = [mi, Kd, Cx0_bat, Cs0_bat, Cp0_bat, tf_bat, Yxs, alfa, beta]
    cond_inic_bat = [entr_val_bat[3],entr_val_bat[4],entr_val_bat[5]]
    t_bat = np.arange(0,tf_bat,0.5)
    entr_val_alim = Cs0_alim, tf_alim, V0, Vf, Q, Q0, a, Q0_exp, beta
    t_alim = np.arange(tf_bat,tf_alim,0.5)
    return(entr_val_bat,cond_inic_bat,t_bat, entr_val_alim, t_alim)

# Função 2)
def modelag_mi_const_dados_conc_sim():
   importado = pd.read_excel("mi_const_relatorio_fapesp_maior_int.xlsx","C_t_exp") 
