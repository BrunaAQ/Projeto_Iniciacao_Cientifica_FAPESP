...
Tfbatelada_cor = Tfbatelada + Intervalobatelada
print(Tfbatelada_cor)
##Entrada para a taxa específica de crescimento:
mimaximo = 0.4
##Entrada para a constante KSX:
KSX = 3.1
...
# Vetor tempo:
tbatelada=np.arange(T0batelada,Tfbatelada_cor,Intervalobatelada)
# Integrando:
...
print("condição inicial alimentada", init_concent_alimentada)
...
print(len(concentotalx), len(concentotals), len(concentotalp), len(Ttotal))

## Exportação dos dados em dataframes:
df_concents = pd.DataFrame({'Tempo(h)': Ttotal, 'Cx_exp(g/L)': concentotalx, 'Cs_exp(g/L)': concentotals, 'Cp_exp(g/L)': concentotalp})
df_params = pd.DataFrame({'mimáx_sim(h-¹)': [mimaximo],'KSX_sim(gp/gs)': [KSX],
                              'Yxs_sim(gcél/gsubs)': [Yxs],
                              'alfa(gprod/gcél)': [alfa], 'beta_sim(gprod/gcél.h)': [beta],
                              "Q(L.h)": [Q], "V0(L)": [V0], "tf_batelada(h)": [Tfbatelada], 
                              "Cs0_alim(gs/L)": [Cs0alimentacao]})
...
## Cálculo mi:
mi = mimaximo*(concentotals/(KSX+concentotals))
...
_ = f = plt.figure() 
_ = ax = f.add_subplot(111)
_ = plt.plot(t_total_ba, V, color = "brown",linewidth=3, label='Volume')
...
## Vazão constante:
Q_const = np.repeat(Q, len(t_total_ba))
## Plotando a figura gráfica - Vazão:  
tam_graf()  
_ = f = plt.figure() 
_ = ax = f.add_subplot(111)
_ = plt.plot(t_total_ba, Q_const, color = "magenta",linewidth=3, label='Vazão')
_ = plt.xlabel('Tempo de cultivo alimentado (h)',weight='bold')               
