...
df_concents_produt = pd.DataFrame({'Tempo(h)': Ttotal, 'Cx(g/L)': concentotalx, 'Cs(g/L)': concentotals, 'Cp(g/L)': concentotalp,'mi(h-¹)':mi,
                           'Px(gcél/L.h)': Px, 'Pp(gprod/L.h)': Pp, 'Ppx(gprod/gcél)':Ppx})
df_vol_vaz = pd.DataFrame({"Tempo(h)": t_total_ba, "Volume(L)": V_calc, "Vazão(L/h)": Q_const})
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)': [mimaximo],'Ks_sim(g/L)': [Ks],
