...
val_ent_rand_Wu_et_al = Modulos_Wu_et_al.entr_rand_Wu_colab()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética de Wu et al.:
def sim_bat_Wu_colab(C,t):
    Cx,Cs,Cp=C
    sim_bat_Wu_colab_val_entr = val_ent_rand_Wu_et_al[0]
    mimaximo = sim_bat_Wu_colab_val_entr[0]
    Ks = sim_bat_Wu_colab_val_entr[1]
    Kd = sim_bat_Wu_colab_val_entr[2]
    Yxs = sim_bat_Wu_colab_val_entr[7]
    alfa = sim_bat_Wu_colab_val_entr[8]
    beta = sim_bat_Wu_colab_val_entr[9]
    KE = sim_bat_Wu_colab_val_entr[10]
    v = sim_bat_Wu_colab_val_entr[11]
...
inic_cond = val_ent_rand_Wu_et_al[1]
t = val_ent_rand_Wu_et_al[2]

# Integrando numericamente:
C = odeint(sim_bat_Wu_colab,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada wu et al. simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    ax2.set_ylabel('Produtuvidade Produto (gp/L.h)', weight='bold') 
    lns = lns1+lns2
    labs = [l.get_label() for l in lns]
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada wu et al. simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada wu et al. simulação produtividade específica.png")                  
imprimir_produtividade_especifica(t, Ppx)

# Cálculo do valor de mi para cada tempo:
mi=val_ent_rand_Wu_et_al[0][0]*(Cs/(val_ent_rand_Wu_et_al[0][1]+Cs+Cs*((Cs/val_ent_rand_Wu_et_al[0][10])**val_ent_rand_Wu_et_al[0][11])))
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada wu et al. simulação variação mi com o tempo.png")
...
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)':[val_ent_rand_Wu_et_al[0][0]],'Ks_sim(g/L)':[val_ent_rand_Wu_et_al[0][1]],
                              'Kd_sim(h-¹)':[val_ent_rand_Wu_et_al[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_Wu_et_al[0][7]],
                              'alfa(gprod/gcél)':[val_ent_rand_Wu_et_al[0][8]], 'beta_sim(gprod/gcél.h)':[val_ent_rand_Wu_et_al[0][9]],
                              'KE (g/L)':[val_ent_rand_Wu_et_al[0][10]], 'v(adim)':[val_ent_rand_Wu_et_al[0][11]]})
df_saida_Wu = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_Wu_et_al.xlsx') as writer:
    df_saida_Wu.to_excel(writer, sheet_name="Saída_Wu_et_al.")
    writer.save()
