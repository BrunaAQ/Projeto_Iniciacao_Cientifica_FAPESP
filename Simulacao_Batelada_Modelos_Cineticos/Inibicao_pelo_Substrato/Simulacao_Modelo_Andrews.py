...
val_ent_rand_Andrews = Modulos_Andrews.entr_rand_Andrews()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética de Andrews:
def sim_bat_Andrews(C,t):
    Cx,Cs,Cp=C
    sim_bat_Andrews_val_entr = val_ent_rand_Andrews[0]
    mimaximo = sim_bat_Andrews_val_entr[0]
    Ks = sim_bat_Andrews_val_entr[1]
    Kd = sim_bat_Andrews_val_entr[2]
    Yxs = sim_bat_Andrews_val_entr[7]
    alfa = sim_bat_Andrews_val_entr[8]
    beta = sim_bat_Andrews_val_entr[9]
    KIS = sim_bat_Andrews_val_entr[10]
...
inic_cond = val_ent_rand_Andrews[1]
t = val_ent_rand_Andrews[2]

# Integrando numericamente:
C = odeint(sim_bat_Andrews,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada andrews simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada andrews simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada andrews simulação produtividade específica.png")                  
imprimir_produtividade_especifica(t, Ppx)

# Cálculo do valor de mi para cada tempo:
mi=val_ent_rand_Andrews[0][0]*(Cs/(val_ent_rand_Andrews[0][1]+Cs+((Cs**2)/val_ent_rand_Andrews[0][10])))
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada andrews simulação variação mi com o tempo.png")
...
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)':[val_ent_rand_Andrews[0][0]],'Ks_sim(g/L)':[val_ent_rand_Andrews[0][1]],
                              'Kd_sim(h-¹)':[val_ent_rand_Andrews[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_Andrews[0][7]],
                              'alfa(gprod/gcél)':[val_ent_rand_Andrews[0][8]], 'beta_sim(gp/gx.h)':[val_ent_rand_Andrews[0][9]], 
                              'KIS(g/L)':[val_ent_rand_Andrews[0][10]]})
df_saida_Andrews = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_Andrews.xlsx') as writer:
    df_saida_Andrews.to_excel(writer, sheet_name="Saída_Andrews")
    writer.save()
