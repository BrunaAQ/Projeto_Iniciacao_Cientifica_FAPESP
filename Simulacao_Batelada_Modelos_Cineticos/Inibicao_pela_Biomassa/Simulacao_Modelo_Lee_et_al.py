...
val_ent_rand_Lee_et_al = Modulos_Lee_et_al.entr_rand_Lee_colab()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética de Lee et al.:
def sim_bat_Lee_colab(C,t):
    Cx,Cs,Cp=C
    sim_bat_Lee_colab_val_entr = val_ent_rand_Lee_et_al[0]
    mimaximo = sim_bat_Lee_colab_val_entr[0]
    Ks = sim_bat_Lee_colab_val_entr[1]
    Kd = sim_bat_Lee_colab_val_entr[2]
    Yxs = sim_bat_Lee_colab_val_entr[7]
    alfa = sim_bat_Lee_colab_val_entr[8]
    beta = sim_bat_Lee_colab_val_entr[9]
    m = sim_bat_Lee_colab_val_entr[10]
    Cx_estr = sim_bat_Lee_colab_val_entr[11]
...
inic_cond = val_ent_rand_Lee_et_al[1]
t = val_ent_rand_Lee_et_al[2]

# Integrando numericamente:
C = odeint(sim_bat_Lee_colab,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada lee et al. simulação.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada lee et al. simulação produtividade específica.png")                  
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada lee et al. simulação variação mi com o tempo.png")
...
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)':[val_ent_rand_Lee_et_al[0][0]],'Ks_sim(g/L)':[val_ent_rand_Lee_et_al[0][1]],
                              'Kd_sim(h-¹)':[val_ent_rand_Lee_et_al[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_Lee_et_al[0][7]],
                              'alfa(gprod/gcél)':[val_ent_rand_Lee_et_al[0][8]], 'beta_sim(gprod/gcél.h)':[val_ent_rand_Lee_et_al[0][9]],
                              'm_sim(adim)':[val_ent_rand_Lee_et_al[0][10]], 'Cx_estr_sim(g/L)':[val_ent_rand_Lee_et_al[0][11]]})
df_saida_Lee = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_Lee_et_al.xlsx') as writer:
    df_saida_Lee.to_excel(writer, sheet_name="Saída_Lee_et_al.")
    writer.save()
