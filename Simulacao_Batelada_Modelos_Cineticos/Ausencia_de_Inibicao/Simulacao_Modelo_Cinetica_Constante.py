...
val_ent_rand_mi_const = Modulos_mi_constante.entr_rand_mi_const()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética constante:
def sim_bat_mi_const(C,t):
    Cx,Cs,Cp=C
    sim_bat_mi_const_val_entr = val_ent_rand_mi_const [0]
    mi = sim_bat_mi_const_val_entr[0]
    Yxs = sim_bat_mi_const_val_entr[1]
    Kd = sim_bat_mi_const_val_entr[2]
    alfa = sim_bat_mi_const_val_entr[7]
    beta = sim_bat_mi_const_val_entr[8]
...
inic_cond = val_ent_rand_mi_const[1]
t = val_ent_rand_mi_const[2]

# Integrando numericamente:
C = odeint(sim_bat_mi_const,inic_cond,t)
...
# Correção - para excluir valores de substrato menores do que 0:
ContadorCs=0
for ContadorCs in range(len(t)):
    if (Cs[ContadorCs]<0):
        Cs[ContadorCs]=0
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada a mi constante simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada a mi constante simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada a mi constante simulação produtividade específica.png")                  
imprimir_produtividade_especifica(t, Ppx)

#Captura do valor de entrada para mi:
mi = np.repeat([val_ent_rand_mi_const[0][0]],len(t))
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada a mi constante simulação variação mi com o tempo.png")
...
df_concents_produt = pd.DataFrame({'Tempo(h)': t, 'Cx(g/L)': Cx, 'Cs(g/L)': Cs, 'Cp(g/L)': Cp,'Px(gcél/L.h)': Px, 
                           'Pp(gprod/L.h)': Pp, 'Ppx(gprod/gcél)':Ppx})
df_params_sim = pd.DataFrame({'mi_sim(h-¹)':[val_ent_rand_mi_const[0][0]],
                              'Kd_sim(h-¹)':[val_ent_rand_mi_const[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_mi_const[0][1]],
                              'alfa(gprod/gcél)':[val_ent_rand_mi_const[0][7]], 'beta_sim(gprod/gcél.h)':[val_ent_rand_mi_const[0][8]]})
df_saida_mi_const = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_mi_constante.xlsx') as writer:
    df_saida_mi_const.to_excel(writer, sheet_name="Saída_mi_constante")
    writer.save()
