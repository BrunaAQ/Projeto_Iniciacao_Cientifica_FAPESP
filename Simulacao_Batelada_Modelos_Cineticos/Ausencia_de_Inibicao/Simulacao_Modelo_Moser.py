...
C = odeint(sim_bat_Moser,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada Moser simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada Moser simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada Moser simulação produtividade específica.png")                  
imprimir_produtividade_especifica(t, Ppx)

# Cálculo do valor de mi para cada tempo:
mi=val_ent_rand_Moser[0][0]*(Cs/(val_ent_rand_Moser[0][1]+Cs))
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada Moser simulação variação mi com o tempo.png")
...
df_saida_Moser = pd.concat([df_concents_concent_produt, df_params_sim], axis=1)
