...
val_ent_rand_Levenspiel = Modulos_Levenspiel.entr_rand_Levenspiel()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética de Levenspiel:
def sim_bat_Levenspiel(C,t):
    Cx,Cs,Cp=C
    sim_bat_Levenspiel_val_entr = val_ent_rand_Levenspiel[0]
    mimaximo = sim_bat_Levenspiel_val_entr[0]
    Ks = sim_bat_Levenspiel_val_entr[1]
    Kd = sim_bat_Levenspiel_val_entr[2]
    Yxs = sim_bat_Levenspiel_val_entr[7]
    alfa = sim_bat_Levenspiel_val_entr[8]
    beta = sim_bat_Levenspiel_val_entr[9]
    n = sim_bat_Levenspiel_val_entr[10]
    Cp_estr = sim_bat_Levenspiel_val_entr[11]
...
inic_cond = val_ent_rand_Levenspiel[1]
t = val_ent_rand_Levenspiel[2]

# Integrando numericamente:
C = odeint(sim_bat_Levenspiel,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada levenspiel simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada levenspiel simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada levenspiel simulação produtividade específica.png")                  
imprimir_produtividade_especifica(t, Ppx)

# Cálculo do valor de mi para cada tempo:
mi=val_ent_rand_Levenspiel[0][0]*((Cs/(val_ent_rand_Levenspiel[0][1]+Cs))*((1-(Cp/val_ent_rand_Levenspiel[0][11]))**val_ent_rand_Levenspiel[0][11]))
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada levenspiel simulação variação mi com o tempo.png")
...
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)':[val_ent_rand_Levenspiel[0][0]],'Ks_sim(g/L)':[val_ent_rand_Levenspiel[0][1]],
                              'Kd_sim(h-¹)':[val_ent_rand_Levenspiel[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_Levenspiel[0][7]],
                              'alfa(gprod/gcél)':[val_ent_rand_Levenspiel[0][8]], 'beta_sim(gprod/gcél.h)':[val_ent_rand_Levenspiel[0][9]], "n_sim(adim)":[val_ent_rand_Levenspiel[0][10]], "Cp*_sim(g/L)":[val_ent_rand_Levenspiel[0][11]]})
df_saida_Levenspiel = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_Levespiel.xlsx') as writer:
    df_saida_Levenspiel.to_excel(writer, sheet_name="Saída_Levenspiel")
    writer.save()
