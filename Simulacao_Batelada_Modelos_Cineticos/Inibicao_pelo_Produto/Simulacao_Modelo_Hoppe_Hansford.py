...
val_ent_rand_Hope_Hansford = Modulos_Hope_Hansford.entr_rand_Hope_Hansford()

# Modelos matemáticos preditos pelo balanço de massa para batelada com cinética de Hoppe & Hansford:
def sim_bat_Hope_Hansford(C,t):
    Cx,Cs,Cp=C
    sim_bat_Hope_Hansford_val_entr = val_ent_rand_Hope_Hansford[0]
    mimaximo = sim_bat_Hope_Hansford_val_entr[0]
    Ks = sim_bat_Hope_Hansford_val_entr[1]
    Kd = sim_bat_Hope_Hansford_val_entr[2]
    Yxs = sim_bat_Hope_Hansford_val_entr[7]
    alfa = sim_bat_Hope_Hansford_val_entr[8]
    beta = sim_bat_Hope_Hansford_val_entr[9]
    Kp = sim_bat_Hope_Hansford_val_entr[10]
    
    mi=mimaximo*(Cs/(Ks+Cs))*(Kp/(Kp+Cp))
    dCxdt=(mi-Kd)*Cx
    dCsdt=(-1/Yxs)*mi*Cx
...
inic_cond = val_ent_rand_Hope_Hansford[1]
t = val_ent_rand_Hope_Hansford[2]

# Integrando numericamente:
C = odeint(sim_bat_Hope_Hansford,inic_cond,t)
...
def imprimir_perfil_concentracao (t_m, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Batelada hoppe e hansford simulação.png")
...
def imprimir_produtividade_celular_produto (t_m, Px_m, Pp_m):
    ...
    plt.style.use('default') 
    plt.show() 
    plt.savefig("Batelada hoppe e hansford simulação produtividade celular e do produto.png")
...
def imprimir_produtividade_especifica (t_m, Ppx_m):
    ...
    plt.grid(True)                                                       
    plt.show()      
    plt.savefig("Batelada hoppe e hansford simulação produtividade específica.png")                  
...
def imprimir_taxa_especifica_crescimento (t_m, mi_m):
    ...
    plt.grid(True)
    plt.show()     
    plt.savefig("Batelada hoppe e hansford simulação variação mi com o tempo.png")
...
df_params_sim = pd.DataFrame({'mimáx_sim(h-¹)':[val_ent_rand_Hope_Hansford[0][0]],'Ks_sim(g/L)':[val_ent_rand_Hope_Hansford[0][1]],
                              'Kd_sim(h-¹)':[val_ent_rand_Hope_Hansford[0][2]], 'Yxs_sim(gcél/gsubs)':[val_ent_rand_Hope_Hansford[0][7]],
                              'alfa(gprod/gcél)':[val_ent_rand_Hope_Hansford[0][8]], 'beta_sim(gprod/gcél.h)':[val_ent_rand_Hope_Hansford[0][9]], "Kp_sim(g/L)":[val_ent_rand_Hope_Hansford[0][10]]})
df_saida_Hope_Hansford = pd.concat([df_concents_produt, df_params_sim], axis=1)
with pd.ExcelWriter('Nome_arquivo_output_Hoppe_Hansford.xlsx') as writer:
    df_saida_Hope_Hansford.to_excel(writer, sheet_name="Saída_Hoppe_Hansford")
    writer.save()
