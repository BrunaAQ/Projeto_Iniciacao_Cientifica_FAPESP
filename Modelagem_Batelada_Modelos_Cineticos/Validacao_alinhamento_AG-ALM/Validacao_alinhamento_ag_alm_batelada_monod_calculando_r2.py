#Batelada_monod_algoritmo_genetico_levemberg.py
# Este programa gera os dados experimentais com valores aleatórios
#Ajuste dos parâmetros com integração - Algortimo genético acoplado com Levemberg-Marquardt
# Este programa utiliza o balanço de massa do biorreator para Células, Substrato e Produto importando os dados experimentais utilizando o otimizador algoritmo genético.
...
import time # contagem de tempo para ver o tempo de finalização do programa
import random
...
# GERANDO OS DADOS EXPERIMENTAIS
#Entrada das variáveis para o modelo - gerando de forma aleatória
...
def sim_batelada_Monod (C_exp, t_exp):
    Cx_exp, Cs_exp, Cp_exp = C_exp
    mi_exp = mimaximo*(Cs_exp/(Ks + Cs_exp))
    dCxdt_exp = (mi_exp - Kd)*Cx_exp
    dCsdt_exp = - (1/Yxs)*mi_exp*Cx_exp
    dCpdt_exp = alfa*mi_exp*Cx_exp + beta*Cx_exp
    return (dCxdt_exp,dCsdt_exp,dCpdt_exp)
#Vetor tempo
Intervalo_exp = 0.6
t_exp= np.arange (0, tf, Intervalo_exp)
#integração numérica
C_exp = odeint(sim_batelada_Monod, cond_inic, t_exp)

for i in range(0,3):
    C_exp_i = C_exp[:,i]
    C_exp[:,i] = C_exp_i + np.random.randn(len(C_exp_i)) * 0.5

#FIM DA GERAÇÃO DOS DADOS EXPERIMENTAIS

# Função objetiva, compara os pontos experimentais com o modelo
dpC= [1,1,1] #peso para cada concentração

# Esta função objetiva é para o Algoritmo genético
def func_ob_ag(parametros, *dados):
    # mimax, ks, alfa, Yxs, beta, kd = parametros
    t_exp,C_exp = dados
    p = tuple(parametros)
    C_sim = odeint(Monod_bat, cond_inic, t_exp, args = p)
    res = C_sim - C_exp
    for i in range(0,3):
        res[:,i] = res[:,i]/dpC[i]
    res = res.flatten()
...
# Os limites para gerar os valores de mimax, ks, alfa e Yxs
limites = [(0, 1),(0, 100),(0, 100),(0, 1),(0, 100),(0, 1)]
...
Intervalo = 0.1
t = np.arange(0, t_exp[-1], Intervalo)

# Integrando com os valores dos parâmetros ajustados
C_otim = odeint(Monod_bat, cond_inic, t, args = (param_otim_lm))
...
# Função para definição das dimensões e padrão estético das figuras geradas:
...
# Dimensões do gráfico - gráfico de duplo eixo
alt = 5
larg = 8
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Concentrações_output_50.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    ax.set_xlabel('Tempo de cultivo (h)',weight='bold')               
    ax.set_ylabel('Produtividade Celular (g/L.h)', weight='bold')
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades.png")
    plt.show()
#impressão do gráfico
...
mi_exp = mimaximo*(C_exp[:,1]/(Ks+C_exp[:,1]))
mi_exp[mi_exp<0] = 0
mi=mimaximo_otim*(C_otim[:,1]/(Ks_otim+C_otim[:,1]))
mi[mi<0] = 0

def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    config_plot() 
    config_estetica_eixo_unico()                                             
    ...
    #plt.show() 
    plt.savefig("velocidade específica de crescimento_output_50.png")    
#impressão do gráfico
imprimir_taxa_especifica_crescimento(t_exp,t,mi_exp,mi)

#Equação que permite calcular a produtividade celular específica (Ppx) modelo e experimental:
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus,t_m, Ppx_ajus, Ppx_m):
    ...
    plt.xlabel('Tempo de cultivo (h)',weight='bold')               
    plt.ylabel('Prod. Específica', weight='bold')
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.17),ncol=2, fancybox=True, shadow=True )                     
    #plt.show() 
    plt.savefig("velocidade específica de crescimento_output_50.png")    
#impressão do gráfico
...
print("O intervalo de confiança de 95% de Ks é {:.4f}".format(ICpar[1]) +' g/L')
print("O valor de alfa é {:.4f}".format(param_otim[2]))
print("O intervalo de confiança de 95% de alfa é {:.4f}".format(ICpar[2]))
print("O valor de Yxs é {:.4f}".format(param_otim[3]))
print("O intervalo de confiança de 95% de Yxs é {:.4f}".format(ICpar[3]))
print("O valor de Kd é {:.4f}".format(param_otim[5])+ ' 1/h')
print("O intervalo de confiança de 95% de Kd é {:.4f}".format(ICpar[5]) +' 1/h')
print("O valor de beta é {:.4f}".format(param_otim[4]))
print("O intervalo de confiança de 95% de beta é {:.4f}".format(ICpar[4]))

#Inserção do primeiro valor de Px e Pp, inserido valor igual ao primeiro
...
print("mimax " + str(param_otim[0]-mimaximo))
print("ks " + str(param_otim[1]-Ks))
print("alfa " + str(param_otim[2]-alfa))
print("Yxs " + str(param_otim[3]-Yxs))
print("beta " + str(param_otim[4]-beta))
print("kd " + str(param_otim[5]-Kd))
...
print("O tempo decorrido do ajuste foi de {:.1f}".format(elapsed)+ " segundos")

# Criação de todos os dataframes que serão utilizados
df_concents= pd.DataFrame({'Tempo(h)': t, 'Cx(g/L)': C_otim[:,0], 'Cs(g/L)': C_otim[:,1], 'Cp(g/L)': C_otim[:,2], 'mi(h-¹)': mi, 'Px(g/L.h)': Px, 'Pp(g/L.h)': Pp})
df_params_exp=pd.DataFrame({'mimax_exp(h-¹)':[mimaximo], 'ks_exp(g/L)':[Ks],'kd_exp(h-¹)':[Kd],'Yxs_exp(gcél/gsub)':[Yxs], 'alfa_exp(gprod/gcél)':[alfa], 'beta_exp(gprod/gcél.h)':[beta], 'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mimax(h-¹)':[param_otim[0]],'ks(g/L)':[param_otim[1]],'kd(h-¹)':[param_otim[5]],'Yxs(gcél/gsub)':[param_otim[3]], 'alfa(gprod/gcél)': [param_otim[2]], 'beta(gprod/gcél.h)':[param_otim[4]]})
df_icpar =pd.DataFrame({'ICmimax':[ICpar[0]],'ICks':[ICpar[1]],'ICkd':[ICpar[5]],'ICYxs':[ICpar[3]], 'ICalfa': [ICpar[2]], 'ICbeta':[ICpar[4]]})   
df_concents_exp= pd.DataFrame({'Tempo_exp(h)': t_exp,'Cx_exp(g/L)': C_exp[:,0],'Cs_exp(g/L)': C_exp[:,1],'Cp_exp(g/L)': C_exp[:,2]})
df_produtiv_exp = pd.DataFrame({'Px_exp(g/L.h)': Px_exp, 'Pp_exp(g/L.h)': Pp_exp})
...
with pd.ExcelWriter('Cálculo_R2_output_50.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Ajuste_acoplado_batelada")
    writer.save()
...
with pd.ExcelWriter('Ajuste_batelada_processos_fermentativos_output_50.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Ajuste_acoplado_batelada")
    writer.save()
