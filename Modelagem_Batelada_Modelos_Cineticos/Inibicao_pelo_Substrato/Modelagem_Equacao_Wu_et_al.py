...
dad_entr_geral = Modulos_Wu_et_al.entr_rand_Wu_colab()
...
said_mod = Modulos_Wu_et_al.modelag_bat_Wu_et_al_dados_conc_sim() 
...
df_saida_wu_et_al = pd.DataFrame({"t_exp(h)":t_exp, "Cx_exp(g/L)": C_exp[:,0], "Cs_exp(g/L)": C_exp[:,1], "Cp_exp(g/L)": C_exp[:,2]})
with pd.ExcelWriter('Dados_sim_bat_Wu_et_al.xlsx') as writer:
    df_saida_wu_et_al.to_excel(writer, sheet_name="C_t_exp")
    writer.save()
 '''   
...
func_args = Modulos_Wu_et_al.modelag_bat_Wu_et_al_func_args()
...
limites = Modulo_peso_limite_AG.limites()[6]
args = (t_exp,C_exp)
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Concentrações model acopl bat Wu et al.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades model acopl bat Wu et al.png")
    plt.show()
#impressão do gráfico
...
KE_otim=param_otim[6]
v_otim=param_otim[7]

#mi=mimaximo*(Cs/(Ks+Cs+Cs*((Cs/KE)**v)))
mi_exp = mimaximo_otim*(C_exp[:,1]/(Ks_otim+C_exp[:,1]+C_exp[:,1]*((C_exp[:,1]/KE_otim)**v_otim)))
mi_exp[mi_exp<0] = 0
mi=mimaximo_otim*(C_otim[:,1]/(Ks_otim+C_otim[:,1]+C_otim[:,1]*((C_otim[:,1]/KE_otim)**v_otim)))
mi[mi<0] = 0
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("Velocidade específica de crescimento model acopl bat Wu et al.png")    
#impressão do gráfico
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus,t_m, Ppx_ajus, Ppx_m):
    ...
    plt.xlabel('Tempo de cultivo (h)',weight='bold')               
    plt.ylabel('Proutividade Específica (gp/gx)', weight='bold')
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.17),ncol=2, fancybox=True, shadow=True )
    plt.grid(True)                     
    plt.show() 
    plt.savefig("Produtividade específica model acopl bat Wu et al.png")    
#impressão do gráfico
...
print("O valor de KE é {:.4f}".format(param_otim[6]) +' g/L')
print("O intervalo de confiança de 95% de KE é {:.4f}".format(ICpar[6]))
print("O valor de v é {:.4f}".format(param_otim[7]))
print("O intervalo de confiança de 95% de v é {:.4f}".format(ICpar[7]))
...
print("KE " + str(param_otim[6]-dad_inic[10]))
print("v " + str(param_otim[7]-dad_inic[11]))
...
df_params_exp=pd.DataFrame({'mimax_exp(h-¹)':[dad_inic[0]], 'Ks_exp(g/L)':[dad_inic[1]],'Kd_exp(h-¹)':[dad_inic[2]],'Yxs_exp(gcél/gsub)':[dad_inic[7]], 'alfa_exp(gprod/gcél)':[dad_inic[8]], 'beta_exp(gprod/gcél.h)':[dad_inic[9]],'KE(g/L)':[dad_inic[10]],'v(adim)':[dad_inic[11]],'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mimax(h-¹)':[param_otim[0]],'Ks(g/L)':[param_otim[1]],'Kd(h-¹)':[param_otim[2]],'Yxs(gcél/gsub)':[param_otim[3]], 'alfa(gprod/gcél)': [param_otim[4]], 'beta(gprod/gcél.h)':[param_otim[5]],'KE(g/L)':[param_otim[6]], 'v(adim)':[param_otim[7]]})
df_icpar =pd.DataFrame({'ICmimax':[ICpar[0]],'ICKs':[ICpar[1]],'ICKd':[ICpar[2]],'ICYxs':[ICpar[3]], 'ICalfa': [ICpar[4]], 'ICbeta':[ICpar[5]], 'ICKE':[ICpar[6]], 'ICv':[ICpar[7]]})   
...
with pd.ExcelWriter('Cálculo_R2_model_acopl_bat_Wu_et_al.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Wu_et_al")
    writer.save()
...
with pd.ExcelWriter('Saída_model_acopl_bat_Wu_et_al.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Wu_et_al")
    writer.save()
