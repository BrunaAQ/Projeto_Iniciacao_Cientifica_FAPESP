...
dad_entr_geral = Modulos_Andrews.entr_rand_Andrews()
...
said_mod = Modulos_Andrews.modelag_bat_Andrews_dados_conc_sim() 
...
df_saida_andrews = pd.DataFrame({"t_exp(h)":t_exp, "Cx_exp(g/L)": C_exp[:,0], "Cs_exp(g/L)": C_exp[:,1], "Cp_exp(g/L)": C_exp[:,2]})
with pd.ExcelWriter('Dados_sim_rand_bat_Andrews.xlsx') as writer:
    df_saida_andrews.to_excel(writer, sheet_name="C_t_exp")
    writer.save()
...
func_args = Modulos_Andrews.modelag_bat_Andrews_func_args()
...
limites = Modulo_peso_limite_AG.limites()[3]
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Concentrações model acopl bat Andrews.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades model acopl bat Andrews.png")
    plt.show()
#impressão do gráfico
...
KSI_otim=param_otim[6]

# mi=mimaximo*(Cs/(Ks+Cs+((Cs**2)/KIS)))
mi_exp = mimaximo_otim*(C_exp[:,1]/(Ks_otim+C_exp[:,1]+((C_exp[:,1]**2)/KSI_otim)))
mi_exp[mi_exp<0] = 0
mi= mimaximo_otim*(C_otim[:,1]/(Ks_otim+C_otim[:,1]+((C_otim[:,1]**2)/KSI_otim)))
mi[mi<0] = 0
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("Velocidade específica de crescimento model acopl bat Andrews.png")    
#impressão do gráfico
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus,t_m, Ppx_ajus, Ppx_m):
    ...
    plt.grid(True)                     
    plt.show() 
    plt.savefig("Produtividade específica  model acopl bat Andrews.png")    
#impressão do gráfico
...
print("O valor de KSI é {:.4f}".format(param_otim[6]) +' g/L')
print("O intervalo de confiança de 95% de KSI é {:.4f}".format(ICpar[6]))
...
print("KSI " + str(param_otim[6]-dad_inic[10]))
...
df_params_exp=pd.DataFrame({'mimax_exp(h-¹)':[dad_inic[0]], 'Ks_exp(g/L)':[dad_inic[1]],'Kd_exp(h-¹)':[dad_inic[2]],'Yxs_exp(gcél/gsub)':[dad_inic[7]], 'alfa_exp(gprod/gcél)':[dad_inic[8]], 'beta_exp(gprod/gcél.h)':[dad_inic[9]], 'KSI_exp(g/L)':[dad_inic[10]],'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mimax(h-¹)':[param_otim[0]],'Ks(g/L)':[param_otim[1]],'Kd(h-¹)':[param_otim[2]],'Yxs(gcél/gsub)':[param_otim[3]], 'alfa(gprod/gcél)': [param_otim[4]], 'beta(gprod/gcél.h)':[param_otim[5]], 'KSI_exp(g/L)':[param_otim[6]]})
df_icpar =pd.DataFrame({'ICmimax':[ICpar[0]],'ICKs':[ICpar[1]],'ICKd':[ICpar[2]],'ICYxs':[ICpar[3]], 'ICalfa': [ICpar[4]], 'ICbeta':[ICpar[5]], 'ICKIS':[ICpar[6]]})   
...
with pd.ExcelWriter('Cálculo_R2_model_acopl_bat_Andrews.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Andrews")
    writer.save()
...
with pd.ExcelWriter('Saída_model_acopl_bat_Andrews.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Andrews")
    writer.save()
