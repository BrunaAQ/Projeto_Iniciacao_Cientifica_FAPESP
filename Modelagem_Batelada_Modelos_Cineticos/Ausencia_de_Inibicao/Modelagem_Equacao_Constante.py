...
dad_entr_geral = Modulos_mi_constante.entr_rand_mi_const()
...
said_mod = Modulos_mi_constante.modelag_bat_mi_const_dados_conc_sim() 
...
df_saida_mi_constante = pd.DataFrame({"t_exp(h)":t_exp, "Cx_exp(g/L)": C_exp[:,0], "Cs_exp(g/L)": C_exp[:,1], "Cp_exp(g/L)": C_exp[:,2]})
with pd.ExcelWriter('Dados_sim_rand_bat_mi_constante.xlsx') as writer:
    df_saida_mi_constante.to_excel(writer, sheet_name="C_t_exp")
'''  
...
func_args = Modulos_mi_constante.modelag_bat_mi_const_func_args()
...
limites = Modulo_peso_limite_AG.limites()[9]

args = (t_exp,C_exp)
resultado = differential_evolution(func_ob_ag, limites, args=args,updating='immediate')
resultado_ag = resultado.x
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    plt.show()
    plt.savefig("Concentrações  model acopl bat mi constante.png")  
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades  model acopl bat mi constante.png")
    plt.show()
#impressão do gráfico
imprimir_produtividade_celular_produto_model_otim_exp(t_exp[1:], t[1:],Px_exp, Pp_exp, Px, Pp)

#PRODUTIVIDADE ESPECÍFICA:
#Calculando Produtividade Específica - Ppx tanto experimental quanto do modelo
Ppx_exp = C_exp[:,1]*(1/C_exp[:,0]) 
Ppx_exp[Ppx_exp<0] = 0 # Se o valor é menor que 0 é substituido por 0

Ppx = C_otim[:,1]*(1/C_otim[:,0]) 
Ppx[Ppx<0] = 0 # Se o valor é menor que 0 é substituido por 0
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus, t_m, Ppx_ajus, Ppx_m):
    ...
    plt.grid(True)                     
    plt.show()
    plt.savefig("Produtividade específica  model acopl bat mi constante.png")    
#impressão do gráfico
...
# Cálculo da variação da taxa mi com o tempo de cultivo - modelo e experimental:
mi = np.repeat(param_otim[0], len(t))
mi_exp = np.repeat(param_otim[0], len(t_exp))
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("Velocidade específica de crescimento model acopl bat mi constante.png")    
#impressão do gráfico
imprimir_taxa_especifica_crescimento(t_exp,t,mi_exp,mi)

print("____Resultado do ajuste do modelo____")
print("O valor de mi é {:.4f}".format(param_otim[0]) +' 1/h')
print("O intervalo de confiança de 95% de mi é {:.4f}".format(ICpar[0]))
print("O valor de Kd é {:.4f}".format(param_otim[1]) +' 1/h')
print("O intervalo de confiança de 95% de Kd é {:.4f}".format(ICpar[1]))
print("O valor de Yxs é {:.4f}".format(param_otim[2]) +' gx/gs')
print("O intervalo de confiança de 95% de Yxs é {:.4f}".format(ICpar[2]))
print("O valor de alfa é {:.4f}".format(param_otim[3]) +' gp/gx')
print("O intervalo de confiança de 95% de alfa é {:.4f}".format(ICpar[3]))
print("O valor de beta é {:.4f}".format(param_otim[4]) + ' gp/gx.h')
...
print("mi " + str(param_otim[0]-dad_inic[0]))
print("Kd " + str(param_otim[1]-dad_inic[1]))
print("Yxs " + str(param_otim[2]-dad_inic[2]))
print("alfa " + str(param_otim[3]-dad_inic[7]))
print("beta " + str(param_otim[4]-dad_inic[8]))
...
df_concents= pd.DataFrame({'Tempo(h)': t, 'Cx(g/L)': C_otim[:,0], 'Cs(g/L)': C_otim[:,1], 'Cp(g/L)': C_otim[:,2], 'Px(gcél/L.h)': Px, 'Pp(gprod/L.h)': Pp, 'Ppx(gprod/gcél)':Ppx})
df_params_exp=pd.DataFrame({'mi_exp(h-¹)':[dad_inic[0]], 'Kd_exp(h-¹)':[dad_inic[2]],'Yxs_exp(gcél/gsub)':[dad_inic[1]], 'alfa_exp(gprod/gcél)':[dad_inic[7]], 'beta_exp(gprod/gcél.h)':[dad_inic[8]],'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mi(h-¹)':[param_otim[0]],'Kd(h-¹)':[param_otim[1]],'Yxs(gcél/gsub)':[param_otim[2]], 'alfa(gprod/gcél)': [param_otim[3]], 'beta(gprod/gcél.h)':[param_otim[4]]})
...
with pd.ExcelWriter('Cálculo_R2_output_mi_constante.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="mi_constante")
    writer.save()
## Valores de tempo, concentração, produtividade, parâmetros cinéticos e erros:   
df_concents = pd.concat([df_concents_exp, df_produtiv_exp, df_concents, df_params_exp, df_params, df_r2], axis=1) 
with pd.ExcelWriter('Ajuste_batelada_processos_fermentativos_output_mi_constante.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="mi_constante")
    writer.save()
