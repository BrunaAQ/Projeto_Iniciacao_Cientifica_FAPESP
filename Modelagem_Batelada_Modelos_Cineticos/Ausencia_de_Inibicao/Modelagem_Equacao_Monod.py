...
dad_entr_geral = Modulos_Monod.entr_rand_Monod()
...
said_mod = Modulos_Monod.modelag_Monod_dados_conc_sim() 
...
func_args = Modulos_Monod.modelag_bat_Monod_func_args()
...
limites = Modulo_peso_limite_AG.limites()[0]
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Concentrações model acopl bat Monod.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades model acopl bat Monod.png")
    plt.show()
#impressão do gráfico
...
mi_exp = mimaximo_otim*(C_exp[:,1]/(Ks_otim+C_exp[:,1]))
mi_exp[mi_exp<0] = 0
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("velocidade específica de crescimento model acopl bat Monod.png")    
#impressão do gráfico
...
#Inserção do primeiro valor de Px e Pp - atribuição de 0 ao tempo inicial:
Px = np.insert(Px,0,0)
Pp = np.insert(Pp,0,0)
...
df_params_exp=pd.DataFrame({'mimax_exp(h-¹)':[dad_inic[0]], 'Ks_exp(g/L)':[dad_inic[1]],'Kd_exp(h-¹)':[dad_inic[2]],'Yxs_exp(gcél/gsub)':[dad_inic[7]], 'alfa_exp(gprod/gcél)':[dad_inic[8]], 'beta_exp(gprod/gcél.h)':[dad_inic[9]], 'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mimax(h-¹)':[param_otim[0]],'Ks(g/L)':[param_otim[1]],'Kd(h-¹)':[param_otim[2]],'Yxs(gcél/gsub)':[param_otim[3]], 'alfa(gprod/gcél)': [param_otim[4]], 'beta(gprod/gcél.h)':[param_otim[5]]})
...
with pd.ExcelWriter('Cálculo_R2_model_acopl_bat_Monod.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Monod")
    writer.save()
...
with pd.ExcelWriter('Saída_model_acopl_bat_Monod.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Monod")
    writer.save()
