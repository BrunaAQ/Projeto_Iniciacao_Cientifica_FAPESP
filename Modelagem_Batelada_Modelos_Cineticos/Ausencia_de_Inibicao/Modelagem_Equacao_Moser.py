...
dad_entr_geral = Modulos_Moser.entr_rand_Moser()
...
said_mod = Modulos_Moser.modelag_bat_Moser_dados_conc_sim() 
...
func_args = Modulos_Moser.modelag_bat_Moser_func_args()
...
limites = Modulo_peso_limite_AG.limites()[2]
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    plt.savefig("Concentrações model acopl bat Moser.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades model acopl bat Moser.png")
    plt.show()
#impressão do gráfico
...
mi_exp = mimaximo_otim*((C_exp[:,1]**u_otim)/(Ks_otim+(C_exp[:,1]**u_otim)))
mi_exp[mi_exp<0] = 0
mi = mimaximo_otim*((C_otim[:,1]**u_otim)/(Ks_otim+(C_otim[:,1]**u_otim)))
mi[mi<0] = 0
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("Velocidade específica de crescimento model acopl bat Moser.png")    
#impressão do gráfico
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus,t_m, Ppx_ajus, Ppx_m):
    ...
    plt.grid(True)                     
    plt.show() 
    plt.savefig("Produtividade específica model acopl bat Moser.png")    
#impressão do gráfico
...
print("O valor de u é {:.4f}".format(param_otim[6]))
print("O intervalo de confiança de 95% de u é {:.4f}".format(ICpar[6]))
...
df_params_exp=pd.DataFrame({'mimax_exp(h-¹)':[dad_inic[0]], 'Ks_exp(g/L)':[dad_inic[1]],'Kd_exp(h-¹)':[dad_inic[2]],'Yxs_exp(gcél/gsub)':[dad_inic[7]], 'alfa_exp(gprod/gcél)':[dad_inic[8]], 'beta_exp(gprod/gcél.h)':[dad_inic[9]],'u_exp(adim)':[dad_inic[10]], 'tempo decorrido(s)':[elapsed]})
df_params =pd.DataFrame({'mimax(h-¹)':[param_otim[0]],'Ks(g/L)':[param_otim[1]],'Kd(h-¹)':[param_otim[2]],'Yxs(gcél/gsub)':[param_otim[3]], 'alfa(gprod/gcél)': [param_otim[4]], 'beta(gprod/gcél.h)':[param_otim[5]],'u(h-¹)':[param_otim[6]]})
df_icpar =pd.DataFrame({'ICmimax':[ICpar[0]],'ICKs':[ICpar[1]],'ICKd':[ICpar[2]],'ICYxs':[ICpar[3]], 'ICalfa': [ICpar[4]], 'ICbeta':[ICpar[5]], 'ICu':[ICpar[6]]})   
...
with pd.ExcelWriter('Cálculo_R2_model_acopl_bat_Moser.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Moser")
    writer.save()
...
with pd.ExcelWriter('Saída_model_acopl_bat_Moser.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Moser")
    writer.save()
