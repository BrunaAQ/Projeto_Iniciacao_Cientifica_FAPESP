...
dad_entr_geral = Modulos_Contois.entr_rand_Contois()
...
said_mod = Modulos_Contois.modelag_bat_Contois_dados_conc_sim() 
...
df_saida_contois = pd.DataFrame({"t_exp(h)":t_exp, "Cx_exp(g/L)": C_exp[:,0], "Cs_exp(g/L)": C_exp[:,1], "Cp_exp(g/L)": C_exp[:,2]})
...
func_args = Modulos_Contois.modelag_bat_Contois_func_args()
...
limites = Modulo_peso_limite_AG.limites()[1]
...
def imprimir_perfil_concentracao_model_otim_exp (t_ajus, t_m, Cx_ajus, Cs_ajus, Cp_ajus, Cx_m, Cs_m, Cp_m):
    ...
    ax.legend(lns, labs, loc='upper center', bbox_to_anchor=(0.5, 1.19),ncol=3, fancybox=True, shadow=True )                                                
    ax.grid(True)                     
    f.set_figheight(alt)                                                 
    f.set_figwidth(larg)                                                   
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Concentrações model acopl bat Contois.png")
    plt.show()
...
def imprimir_produtividade_celular_produto_model_otim_exp (t_ajus, t_m, Px_ajus, Pp_ajus, Px_m, Pp_m):
    ...
    f.patch.set_facecolor('white')                                   
    plt.style.use('default')    
    plt.savefig("Produtividades model acopl bat Contois.png")
    plt.show()
#impressão do gráfico
...
KSX_otim=param_otim[1]

#mimaximo*(Cs/(KSX*Cx+Cs))
mi_exp = mimaximo_otim*(C_exp[:,1]/(KSX_otim*C_exp[:,0]+C_exp[:,1]))
mi_exp[mi_exp<0] = 0
mi=mimaximo_otim*(C_otim[:,1]/(KSX_otim*C_otim[:,0]+C_otim[:,1]))
mi[mi<0] = 0
...
def imprimir_taxa_especifica_crescimento (t_ajus,t_m, mi_ajus, mi_m):
    ...
    plt.grid(True)                   
    plt.show() 
    plt.savefig("Velocidade específica de crescimento model acopl bat Contois.png")    
#impressão do gráfico
...
def imprimir_produtividade_especifica_model_otim_exp (t_ajus,t_m, Ppx_ajus, Ppx_m):
    ...
    plt.grid(True)                     
    plt.show() 
    plt.savefig("Produtividade específica model acopl bat Contois.png")    
#impressão do gráfico
...
#Inserção do primeiro valor de Px, Pp e Ppx - valor igual ao primeiro;
Px = np.insert(Px,0,0)
Pp = np.insert(Pp,0,0)
...
print("KSX " + str(param_otim[1]-dad_inic[1]))
...
with pd.ExcelWriter('Cálculo_R2_model_acopl_bat_Contois.xlsx') as writer:
    df_saida_compar_comp.to_excel(writer, sheet_name="Contois")
    writer.save()
...
with pd.ExcelWriter('Saída_model_acopl_bat_Contois.xlsx') as writer:
    df_concents.to_excel(writer, sheet_name="Contois")
    writer.save()
